...
def get_key():
    global key
    global easy_list
    get_key_list = list(term_dict)
    a = False
    while not a:
        key = random.choice(get_key_list)
        if key in easy_list:
            # debugging
            try:
                get_key_list.pop(get_key_list.index(key))
            except:
            ...
            a = True
    #key = random.choice(list(term_dict))
    #if key in easy_list:
    #    get_key()
...
while not finish:
    ...
    if totalplay > rank_start and totalplay % 5 == 0 and wrong_list: # shows a term from the wrong list
        ...
        if len(easy_list) == len(term_dict):
            finish = True
            print('Everything is now in the easy dictionary')
            break
        else:
            get_key() # there is an exception occuring here RecursionError: maximum recursion depth exceeded while calling a Python object
    ...
    if answer in value:  # default correct
        ...
        if contest == "1" or contest == "y" or contest == "" or contest == "e":  # you got it wrong but you can contest the answer
            correct_answer()
            if not flashcard_mode:
                term_dict[key].append(answer) # smart learning
            else: # if flashcard mode and e
                if contest == "e":
                    easy_list.append(key)
                    try:
                        wrong_list.remove(key)
                    except:
                        pass
                    print('Okay, ' + str(key) + ' will not be shown for a while')
                    pp_dict[key] = pp_dict[key] - pp_easy
